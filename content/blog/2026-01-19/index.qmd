...
excerpt: "La [convocatoria al curso gratuito de introducción a R](/blog/curso_gratis_r_intro_1/) fue un éxito! **Más de 500 personas** postularon para poder aprender.
...
cupos |> 
  ...
  mutate(genero = case_match(genero,
                            ...
                            "queer_ag_nero_otro" ~ "Queer/Agénero/Otro")) |>
  ggplot() +
  aes(x = n, y = genero) +
  geom_col() +
  scale_x_continuous(expand = c(0.01, 0.1)) +
  labs(y = "Género", x = "Cupos asignados")
```
...
cupos |> 
  count(pais) |> 
  mutate(pais = case_match(pais ,
                           "per" ~ "perú",
                           "m_xico" ~ "méxico",
                           .default = pais)) |> 
  mutate(pais = stringr::str_to_title(pais)) |> 
  mutate(pais = forcats::fct_reorder(pais, n)) |>
  ggplot() +
  aes(x = n, y = pais) +
  geom_col() +
  scale_x_continuous(expand = c(0.01, 0.1)) +
  labs(y = "País", x = "Cupos asignados")
```
